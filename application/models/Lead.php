...
//            exit;
    ...
    public function unassigned_leads($lead_status = '',$id = ''){
        ...
        $this->db->select('db_leads.*,db_master_products.title as product_title,db_master_products.map_with as mapping,db_master_products.map_with_amount as mapping_amount');
...
//            return $this->db->where($where)->count_all_results($table);
            return $this->counts($table,$select,$where,$join,$group_by);
              ...
              //pe($this->lists_testing($table,$select,$where,$join,$group_by,$order_by));die;
    ...
    public function unassigned_leads_api($lead_status = '',$branch_id = ''){
        if($lead_status == 'walkin'){
            $this->db->select('db_leads.*,db_branch.name as generated_by,DATEDIFF(CURDATE( ),db_leads.created_on) as elapsed_day,db_master_products.title as product_title,db_master_products.map_with,db_master_products.map_with_amount');
            $this->db->from('db_leads');
            $this->db->join('db_branch','db_branch.code = db_leads.created_by_branch_id ','');
        }else{
            $this->db->select('db_leads.*,DATEDIFF(CURDATE( ),db_leads.created_on) as elapsed_day,db_master_products.title as product_title,db_master_products.map_with,db_master_products.map_with_amount');
            $this->db->from('db_leads');
        }
        $this->db->join('db_lead_assign','db_lead_assign.lead_id = db_leads.id ','left');
        $this->db->join('db_master_products','db_master_products.id = db_leads.product_id ','left');
